...
{
    public function test_cancel_request_definition_has_correct_method()
    {
        $order = new Order();
        $cancel = new Cancel($order);

        $this->assertEquals('POST', $cancel->method());
    }

    public function test_cancel_request_definition_has_correct_url()
    {
        Config::set('istoy.providers.smm.host', 'https://smmlite.com/api/v2');
        
        $order = new Order();
        $status = new Cancel($order);

        $this->assertEquals('https://smmlite.com/api/v2', $status->url());
    }

    public function test_cancel_payload_for_single_order()
    {
    ...
    public function test_cancel_payload_for_collection()
    {
        Config::set('istoy.providers.smm.key', 'test_api_key');
    ...
    public function test_cancel_payload_filters_null_external_ids()
    {
        Config::set('istoy.providers.smm.key', 'test_api_key');
    ...
    public function test_cancel_validation_rules_are_correct()
    {
        $order = new Order();
        $cancel = new Cancel($order);
        $rules = $cancel->rules();
